...
def login():
    ...
    Başarılıysa online_users tablosuna kayıt açar.
    ...
    # ⭐ Güvenlik: Kullanıcı bulunamadı ve şifre yanlış durumunda
    # AYNI hata mesajını ver. Farklı mesaj versek saldırgan
    # hangi kullanıcı adının geçerli olduğunu anlayabilir. (User Enumeration)
    ...
    # IP adresini al
    # X-Real-IP: Nginx reverse proxy arkasında gerçek istemci IP'si bu header'da gelir
    # Yoksa direkt bağlantı IP'sini kullan
...
def logout():
    ...
    online_users tablosundan kaydı siler.
